    ...
    class LatticeSignatureScheme:
        def __init__(self, n=64, q=3329, m1=128, m2=128, m3=32):
            ...
            # Random public matrix D to hide/commit the message
            self.D = np.random.randint(0, q, size=(n, m3))

            # Gadget matrix G = [I_n | 0] of size n x m2
            self.G = np.eye(n, m2)

            # Gaussian parameters for sampling
            self.sigma = 2000        # parameter for G-sampling
            self.sigma1 = 100000     # parameter for perturbation

            # Verification bounds (infinity norm)
            # v1 = p + R*z : |p| ~ sigma1, |R*z| ~ m2 * sigma (since R in {-1,0,1})
            # v2 = z : |z| ~ sigma
        ...
        def keygen(self):
            ...
            - A : random public matrix (n x m1)
            - R : trapdoor (m1 x m2), entries in {-1, 0, 1}
            - B = A*R mod q : derived matrix
            - u : random public vector (n x 1)
            """
    ...
    if __name__ == "__main__":
        ...
        # 2. The authority generates its keys
        print("[2] Keygen...")
        ...
        # 3. Message to sign (e.g., user's age in binary)
        my_message = np.random.randint(0, 2, size=(crypto_system.m3, 1))
        print(f"[3] Message: binary vector of size {my_message.shape}")

        # 4. The authority signs the message
        tag_id = 42
        print(f"[4] Signing in progress (tag={tag_id})...")
        ...
        print(f"    ||v1||_inf = {np.max(np.abs(v1)):.0f}  (bound: {crypto_system.sigma1_bound})")
        print(f"    ||v2||_inf = {np.max(np.abs(v2)):.0f}  (bound: {crypto_system.sigma_bound})")
        print(f"    |v| total  = {v.shape[0]} components")
        ...
        # 7. Unforgeability test
        print("\n[6] Unforgeability test...")
        ...
        print(f"    Verification with fake message : {is_valid_fake}")
        if not is_valid_fake:
            print("    >>> Fake message correctly rejected <<<")
